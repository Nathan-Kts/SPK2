...
double smallest_possible(double input){
    ...
    for (int i = 2; i <= root; i++) {
        if(input_as_int%i==0){
            if(i>biggest_divisor)
                biggest_divisor = i;
            if(input_as_int/i > biggest_divisor)
                biggest_divisor = input_as_int/i;
        }
    }
    return input/biggest_divisor;
}
...
double best_possible(double input){
    ...
    for (int i = 2; i <= root; i++) {
        if(input_as_int%i==0){
            if(abs((input_as_int/i)-objectif)<error) {
                best_divisor = i;
                error = abs((input_as_int / i) - objectif);
            }
            if(abs((input_as_int/(input_as_int/i))-objectif) < error){
                best_divisor = input_as_int/i;
                error = abs((input_as_int/(input_as_int/i))-objectif);}
        }
    }
    return input/best_divisor;
}
...
void public_to_new_private_start_ortho(int nbr_vectors, int vector_size, double received_public_lattice[nbr_vectors][vector_size], double new_private_lattice[nbr_vectors][vector_size]){
    ...
    //printf("The norm is : %f\n", norm(nbr_vectors, nbr_vectors, new_private_lattice));
    //print_matrix(nbr_vectors, nbr_vectors, new_private_lattice);
}